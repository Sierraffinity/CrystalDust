...
static void PrintCurrentMonRibbonCount(struct PokenavSub14 *structPtr)
{
    ...
    AddTextPrinterParameterized3(structPtr->ribbonCountWindowId, 2, 0, 1, color, -1, gStringVar4);
...
static void PrintRibbonNameAndDescription(struct PokenavSub14 *structPtr)
{
    ...
    if (ribbonId < FIRST_GIFT_RIBBON)
    {
        ...
        for (i = 0; i < 2; i++)
            AddTextPrinterParameterized3(structPtr->ribbonCountWindowId, 2, 0, (i * 16) + 1, color, -1, gRibbonDescriptionPointers[ribbonId][i]);
    }
    else
    {
        ...
        ribbonId--;
        for (i = 0; i < 2; i++)
            AddTextPrinterParameterized3(structPtr->ribbonCountWindowId, 2, 0, (i * 16) + 1, color, -1, gGiftRibbonDescriptionPointers[ribbonId][i]);
    }
...
static void PrintRibbonsMonListIndex(struct PokenavSub14 *structPtr)
{
    ...
    *(txtPtr++) = CHAR_SLASH;
    ...
    AddTextPrinterParameterized(structPtr->listIdxWindowId, 2, gStringVar1, x, 1, TEXT_SPEED_FF, NULL);
