...
bool32 UpdateConditionGraphWindows(u8 mode, u16 bufferIndex, bool8 winMode)
{
    ...
    case 1:
        if (GetConditionGraphCurrentMonIndex() != GetMonListCount() - 1 || IsConditionMenuSearchMode() == TRUE)
        {
            ...
            AddTextPrinterParameterized(structPtr->nameGenderWindowId, 2, str, 0, 1, 0, NULL);
        }
        break;
    case 2:
        if (IsConditionMenuSearchMode() == TRUE)
        {
            ...
            AddTextPrinterParameterized(structPtr->nameGenderWindowId, 2, str, 0, 17, 0, NULL);
            ...
            AddTextPrinterParameterized(structPtr->listIndexWindowId, 2, text, 4, 1, 0, NULL);
            ConvertIntToDecimalStringN(text + 5, GetConditionMonDataBuffer(), STR_CONV_MODE_RIGHT_ALIGN, 4);
            AddTextPrinterParameterized(structPtr->listIndexWindowId, 2, text, 28, 1, 0, NULL);
        }
        break;
    case 3:
