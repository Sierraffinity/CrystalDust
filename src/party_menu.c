...
static void CursorCb_Enter(u8 taskId)
{
    ...
    PartyMenuRemoveWindow(&gUnknown_0203CEC4->windowId[0]);
    PartyMenuRemoveWindow(&gUnknown_0203CEC4->windowId[1]);
    if (facility == FRONTIER_FACILITY_BUG_CATCHING_CONTEST)
    {
        PlaySE(SE_SELECT);
        if (gSelectedOrderFromParty[0] != 0)
            DisplayPartyPokemonSelectionText(6, &gUnknown_0203CEDC[gSelectedOrderFromParty[0] - 1], 1);

        gSelectedOrderFromParty[0] = gUnknown_0203CEC8.slotId + 1;
        DisplayPartyPokemonSelectionText(13, &gUnknown_0203CEDC[gUnknown_0203CEC8.slotId], 1);
        Task_ClosePartyMenu(taskId);
...
static void CursorCb_NoEntry(u8 taskId)
{
    ...
    for (i = 0; i < (unk - 1); i++)
    {
        if (gSelectedOrderFromParty[i] != 0)
        {
            if (facility == FRONTIER_FACILITY_BUG_CATCHING_CONTEST) // Bug catching contest
                DisplayPartyPokemonSelectionText(13, &gUnknown_0203CEDC[gSelectedOrderFromParty[i] - 1], 1);
            else
