...
{
    SetTVMetatilesOnMap(gBackupMapLayout.width, gBackupMapLayout.height, 0x35);
    DrawWholeMapView();
}

void TurnOnTVScreen(void)
{
    SetTVMetatilesOnMap(gBackupMapLayout.width, gBackupMapLayout.height, 0x61);
    DrawWholeMapView();
}
...
void PutPokemonTodayFailedOnTheAir(void)
{
    ...
    if (!rbernoulli(1, 1))
    {
        for (i = 0, ct = 0; i < ARRAY_COUNT(gBattleResults.catchAttempts); i ++)
        {
            ct += gBattleResults.catchAttempts[i];
        }
        if (ct > 0xFF)
