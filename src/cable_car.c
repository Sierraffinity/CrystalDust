...
struct CableCar
{
    ...
    /*0x40FC*/ u16 *SlowpokeWell_B1FTilemap;
...
const u8 gCableCarSlowpokeWell_B1FTilemap[] = INCBIN_U8("graphics/misc/cable_car_mt_chimney_map.bin.lz");
...
static void CableCarMainCallback_Setup(void)
{
    ...
    case 2:
        ...
        sCableCar->SlowpokeWell_B1FTilemap = malloc_and_decompress(gCableCarSlowpokeWell_B1FTilemap, &sizeOut);
    ...
    case 7:
        ...
        CopyToBgTilemapBufferRect_ChangePalette(0, sCableCar->SlowpokeWell_B1FTilemap +  0x48,  0, 14, 12, 3, 17);
        CopyToBgTilemapBufferRect_ChangePalette(0, sCableCar->SlowpokeWell_B1FTilemap +  0x6C, 12, 17, 12, 3, 17);
        CopyToBgTilemapBufferRect_ChangePalette(0, sCableCar->SlowpokeWell_B1FTilemap +  0x90, 24, 20, 12, 3, 17);
        CopyToBgTilemapBufferRect_ChangePalette(0, sCableCar->SlowpokeWell_B1FTilemap +   0x0,  0, 17, 12, 3, 17);
        CopyToBgTilemapBufferRect_ChangePalette(0, sCableCar->SlowpokeWell_B1FTilemap +  0x24,  0, 20, 12, 3, 17);
        CopyToBgTilemapBufferRect_ChangePalette(0, sCableCar->SlowpokeWell_B1FTilemap +   0x0, 12, 20, 12, 3, 17);
        CopyToBgTilemapBufferRect_ChangePalette(0, sCableCar->SlowpokeWell_B1FTilemap +  0x24, 12, 23, 12, 3, 17);
        CopyToBgTilemapBufferRect_ChangePalette(0, sCableCar->SlowpokeWell_B1FTilemap +   0x0, 24, 23, 12, 3, 17);
        gMain.state++;
        break;
    case 8:
...
static void CleanupCableCar(void)
{
    ...
    FREE_AND_SET_NULL(sCableCar->SlowpokeWell_B1FTilemap);
...
static void LoadCableCarSprites(void)
{
    ...
    switch (gSpecialVar_0x8004)
    {
        ...
        default:
            ...
            break;
        case 1:
            CopyToBgTilemapBufferRect_ChangePalette(0, sCableCar->SlowpokeWell_B1FTilemap + 0x24, 24, 26, 12, 3, 17);
...
void sub_8151088(void)
{
    ...
    for (i = 0, k = 0, offset = 0x24 * (sCableCar->unk1B + 2); i < 3; i++)
    {
        for (j = 0; j < 12; j++)
        {
            sCableCar->unk22[i][j] = sCableCar->SlowpokeWell_B1FTilemap[offset++];
            sCableCar->unk22[i + 3][j] = sCableCar->SlowpokeWell_B1FTilemap[k];
            sCableCar->unk22[i + 6][j] = (sCableCar->SlowpokeWell_B1FTilemap + 0x24)[k];
            k++;
        }
    }
