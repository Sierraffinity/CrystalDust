...
static void DrawPokeblockMenuTitleText(void)
{
    ...
    PrintOnPokeblockWindow(WIN_TITLE, itemName, GetStringCenterAlignXOffset(2, itemName, 0x48));
...
static void ShowPokeblockActionsWindow(u8 taskId)
{
    ...
    InitMenuInUpperLeftCornerPlaySoundWhenAPressed(tWindowId, 2, 0, 1, 16, sPokeblockMenu->numActions, 0);
...
static void PokeblockAction_Toss(u8 taskId)
{
    ...
    DisplayMessageAndContinueTask(taskId, WIN_TOSS_MSG, 10, 13, 2, GetPlayerTextSpeedDelay(), gStringVar4, CreateTossPokeblockYesNoMenu);
}
...
static void TossedPokeblockMessage(u8 taskId)
{
    ...
    DisplayMessageAndContinueTask(taskId, WIN_TOSS_MSG, 10, 13, 2, GetPlayerTextSpeedDelay(), gStringVar4, TossPokeblock);
}
